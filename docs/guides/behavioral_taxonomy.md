...
- **Match the shape the models were fine-tuned on.** This is the one that fails silently. Adapters fit on bare answer prose and then probed with a *question* to continue are being measured on an input shape that never appeared in training: no error, fluent output, and distances that can invert the ordering you are trying to recover. The yahoo adapters did exactly this and behavioral returned the mixing order backwards (r = −0.9995). Check the training projection — `text_projection` in an adapter's `experiment_meta.json`, or `text_fields` / `text_field` in its recipe — and build probes in that shape. See [Dataset recipes](../api_reference.md#dataset-recipes) for how a recipe composes a row into training text, and note that a probe recipe is normally *uncomposed* (bare question titles) precisely so it forms the prefix of the composed training string.
