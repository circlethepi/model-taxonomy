...
| `nemo` | `mistralai/Mistral-Nemo-Instruct-2407` | `question_only` | Chat prompts at full parity with `qwen`. The 12B rung: with `olmo2` it turns the suite family into a scale ladder (1B / 4B / 8B / 12B) as well as a family comparison. Adapter capacity diverges — 19.7M against the 12–13M band — which the profile's `expected_lora_params` and `notes` declare rather than tune away. Sets `prefetch_ignore` because this checkpoint publishes its weights twice. |
| `olmo2` | `allenai/OLMo-2-0425-1B-Instruct` | `question_only` | **Generated but NOT submittable — do not run `submit_all.sh`.** Its rendered training prompt is the bare string `<|endoftext|>`: the question is discarded by `_trim_to_last_atomic`, because OLMo-2's `<\|user\|>`/`<\|assistant\|>` markers are plain text and the only added-vocab token in the prompt is the BOS at index 0. See the `OLMO2` profile notes for the full diagnosis and the two candidate resolutions. Chat prompts at full parity with `qwen`. The 1B rung, and a family not otherwise represented. Adapter capacity is 4.2M; no ~1B model reaches the 12–13M band at any sensible rank, so this divergence is inherent to a scale ladder. Walls and memory are the `Suite()` defaults, deliberately untuned until a first run measures them. |
