...
| `olmo2` | `allenai/OLMo-2-0425-1B-Instruct` | `question_only` | Chat prompts at full parity with `qwen`. The 1B rung, and a family not otherwise represented. Its template emits `<\|user\|>`/`<\|assistant\|>` as ordinary text rather than as registered tokens, so the profile declares `prompt_end_token=None` and the prompt is the template's full render — this is the checkpoint that showed the old derived cut point was wrong. Adapter capacity is 4.2M; no ~1B model reaches the 12–13M band at any sensible rank, so this divergence is inherent to a scale ladder. Walls and memory are the `Suite()` defaults, deliberately untuned until a first run measures them. |
