...
`crosslevel_agreement_dataset_cosine.md`, `crosslevel_agreement_procrustes.md`,
`functional_layers.md` and `crosslevel_scores.csv`. The *full* per-level ranking is
written once per suite, however many cross-level variants a run writes — in
`crosslevel_scores.csv`, and as the `## Every surrogate, per level` section of the
unsuffixed `crosslevel_agreement.md`. A suffixed agreement file is only its winners
table plus a pointer back here: a variant picks different rows *out of* that ranking,
whether by pinning a level's metric or by ranking on Procrustes instead of dCor, and
neither changes how any row scores. The PNGs are gitignored,
and are regenerated by the command above. These tables are *generated artifacts*: edit the script, not the markdown.
...
the unrestricted one rather than overwriting it. That variant file holds its winners
table and nothing else: the ranking the winners are drawn from is the same ranking, and
is kept in the unsuffixed file.
