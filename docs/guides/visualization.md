...
`crosslevel_scores.csv`. There is exactly one scores CSV per suite, however many
cross-level variants a run writes: it holds the *full* per-level ranking, which the
`dataset_embedding`-pinned-to-`cosine` variant does not change. The PNGs are gitignored,
