...
`crosslevel_agreement_dataset_cosine.md`, `functional_layers.md` and
`crosslevel_scores.csv`. The *full* per-level ranking is written once per suite, however
many cross-level variants a run writes — in `crosslevel_scores.csv`, and as the
`## Every surrogate, per level` section of the unsuffixed `crosslevel_agreement.md`. A
suffixed agreement file is only its winners table plus a pointer back here, because the
`dataset_embedding`-pinned-to-`cosine` variant picks a different row *out of* that
ranking without changing how any row scores. The PNGs are gitignored,
