...
| ~~6~~ | ~~Add distance correlation (dCor); demote Mantel's p-value to descriptive~~ | [distance_matrix_comparison.md](distance_matrix_comparison.md) §1 | **Done.** `distance_correlation`/`dcor_test`/`DcorResult` in `src/analysis/matrices.py`; `compare_taxonomies` reports `dcor_vs_truth` + `dcor_p_value` (table 9 → 11 columns); Mantel's **p-value** carries a warning while its **statistic** is untouched, as this row required. `bias_corrected=True` is the default and is not cosmetic — the classical V-statistic averages **0.85** between two *independent* five-model matrices, so it cannot distinguish signal from nothing here; `dCor*` (Székely & Rizzo 2013) averages ~0. `n! <= n_permutations` triggers exact enumeration, so every p-value on a five-model slice is exact rather than sampled. **Three things the note did not anticipate.** (a) **dCor is unsigned** — a taxonomy recovering the mixing order exactly backwards scores 1.0 — so it does not subsume the correlation, which strengthens this row's "do not drop Mantel". (b) **The p-value has a symmetry floor set by the reference, not the taxonomy:** any relabelling mapping the U-centred reference onto itself gives a tied null value, so `p >= #Aut(B)/n!` for *any* input. The ground truth here (five evenly-spaced mixtures) has 2 automorphisms raw and **8** U-centred, so an exact recovery scores `8/120 ≈ 0.067` and **nothing on these slices reaches `p < 0.05`** — a design limit, not a weak result, and it applies to PROTEST for the same reason. Not universal at n=5: an unevenly-spaced truth has 4 and reaches 0.033. **Rank the levels by the statistic.** (c) **`matrix_corr_vs_truth` and the recovery correlation are different quantities and had been conflated** — the `r = −0.9995` for behavioral in the 2026-08-05 table below is the *recovery* correlation, downstream of MDS + barycentric projection; its matrix correlation is **+0.7622**. At the matrix level no statistic sees direction at all, signed or not. Verified table on the (1000, 0) slice, cosine, layer 27, behavioral greedy `1r`: dataset_embedding `dCor*` 0.9667 / p 0.0667, functional 0.9285 / 0.0667, structural 0.7537 / 0.1000, behavioral 0.3870 / 0.2667. Pinned by `t_dcor_bias`, `t_dcor_test`, `t_dcor_unsigned`, `t_dcor_u_centering_symmetry`. |
...
| 17 | **`fine_tuning.total_train_samples` is opt-in; make it the default** | this row; see also 8 | **Half done — the mechanism exists, the default flip does not.** Training length was only `n_epochs`, so across an `n_samples` sweep the dataset size and the optimizer-step count moved together and no difference between adapters could be attributed to the data alone: at `n=1` a model saw 3 samples, at `n=1000` it saw 3000. `total_train_samples` now sets the total independently — `null` (off, train for `n_epochs`), an integer, or `"auto"` = one epoch of the full source data behind the recipe (`compute_recipe_source_size` in `scripts/_utils.py`: the summed split length, or the summed **full** size of every class used — deliberately a sum, unlike `compute_recipe_capacity`'s `min_c(size_c/w_c)`, and the two differ whenever class weights are unequal: 280000 vs 186666 on `yahoo_75t0_25t1`). Expressed to `SFTTrainer` as `max_steps`, so the realized count quantizes to the effective batch (16). Budgeted adapters get a `_b{samples_seen}` directory suffix — the realized count, applied after the Trainer exists, since `n_gpu` (a factor of the effective batch) is rewritten during its construction — without it two runs differing only in training length collide on one directory and the second is skipped as already-trained; three `_ADAPTER_DIR_RE`s parse that name (`src/analysis/discovery.py`, `src/analysis/identity.py`, `scripts/migrate_recipe_identity.py`) and all three had to learn the segment. `CacheEntry` gained `samples_seen`/`n_epochs`, so `filter()` and `slices()` select on training length at every level; adapters predating this fall back to `n_samples * n_epochs`, which is the same quantity, so the axis is already populated across the whole cache with no retrain. **Still open:** flipping the default to on. `"auto"` on the current Yahoo configs is 140k–280k samples against 3000 today, a 47–93x step increase, so the flip is a compute decision, not a code one — check `--dry-run` output and the Slurm budget before making it. **Also open:** whether the existing `n_samples`-sweep results should be re-run at a fixed budget, which is the measurement this feature was built for. |
| 18 | **Projection is part of `recipe_hash`, so one row-set projected two ways is two directories** | this row; see also 2, 11, 13 | **Not started — noted deliberately rather than done, and the "just remove it from the hash" version of this is a trap.** `text_fields`/`text_separator` feed `recipe_hash`, so `[question_title, question_content, best_answer]` and `[question_title, question_content]` over the same mixture are two recipes with two directories — whose `indices` lists are **byte-identical**. Verified, not assumed: a draw stores source indices rather than rows (schema 2), projection happens at rehydration via `row_text`, row selection depends only on seed / `class_filter` / weights / `n`, and the seed is the plain integer, **not** derived from `recipe_hash` (`src/datasets/mixed_dataset.py:243, 316`). So the two draws genuinely select the same rows. The simplex3 experiment pays this cost once, for the query-set-B ablation: one extra n=100 manifest, ~25 KB. **The principled split** is `recipe_hash` = row selection only, with composition moved into each *consumer's* key — adapter identity, the `02_dataset_embeddings` surrogate key, and the functional/behavioral `query_key`. Projection is a property of how a consumer reads a sample, not of which rows were sampled, which is what makes the current placement feel redundant. **Why it must be one atomic change:** removing composition from the recipe *without* adding it to the consumers makes two adapters trained on identical rows with different projections **indistinguishable** — item 11's exact failure, recreated by construction. The duplication being removed is currently the mechanism preventing it. **It is a migration, not an additive change**, so it gets none of rule 2's protection (`src/datasets/_text_projection.py:23-28`): **5 of the 11 recipes in `01_datasets` carry `text_fields`** (all five `yahoo_qa_*`), backing 5 adapters plus their `04`/`05` entries, and every one of those hashes moves. The pinned hashes in `check_analysis.py` would have to be rewritten, removing the safety net exactly when it is most needed. **Verdict:** worth doing eventually, on its own branch, with the `yahoo_qa_*` adapters re-keyed deliberately — not folded into an experiment. 25 KB is not a reason to run a live identity migration. |
