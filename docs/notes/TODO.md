...
| 15 | **`02_dataset_embeddings` hides the draw inside a hash, unlike every other stage** | [dataset_embedding_layout.md](dataset_embedding_layout.md) | Every other stage spells the draw out in the path — `01_datasets` as a file `n{n}_s{seed}.json`, `04`/`05` as a directory of the same name. `02` folds `n_samples` and `seed` into `embedder_hash` instead, so no `n*_s*` component exists anywhere under it. That was a real fix, not an oversight: once `recipe_hash` went content-addressed under 2, seed left the recipe name and had to go somewhere or every seed would collapse onto one entry. **But all 520 entries measured: `representation`, `model_name` and `prompt_prefix` never vary — only `n_samples` (17) and `seed` (10) do. The hash is de facto a draw hash.** Re-keying to `{recipe_hash}/n{n}_s{seed}/{embedder_hash}/` is **content-preserving — no GPU, no recomputation** — since each `config.json` already records what the new path needs. Aligns the *draw* coordinate only: the dataset level has no model axis, so it can never share the `{base}/{adapter}/` prefix. The note also records two stale docstrings in `src/taxonomy/dataset_embedding.py` that correct item 12. |
