...
Analysis D -- the summaries scored against truth  (``initsweep_truth.csv``)
--------------------------------------------------------------------------
Analysis A scores the ten seeds one at a time.  This scores the objects those
ten *summarise*, so a figure can draw a centre with the ten around it as a
range.  Four sources, and they are four different objects:

``overlay_mean``
    ``seed_mean``: the mean of the ten aligned per-seed fits.  The centre of
    exactly the cloud analysis A's ten values form, which is why it is the one
    the range band belongs around.

``pool_mean_embedding``
    ``mean_after``: the joint 160-model fit's sixteen per-mixture centroids.
    The pooled counterpart of ``overlay_mean`` -- the same operation, "take the
    mean embedding", applied to the other of the two fits.

``pool_mean_distance``
    The 160x160 averaged down to one row per mixture (:func:`mixture_block_means`).
    A mean *distance matrix* rather than a mean embedding, and so the pooled read
    dCor* can take on its own full-dimensional terms; the mean embedding is a 2-D
    configuration and would put dCor* on a footing analysis A is not on.

``pool160``
    The pool itself, against a **160-point truth** -- one that gives every pair
    of seed-siblings a distance of exactly 0, because they share a recipe.  The
    degeneracy that rules the pool out of a Procrustes fit is the whole content
    of this row for dCor*: the truth asserts that initialisation does not exist,
    and the score says how far the surrogate is from agreeing.  That is analysis
    B's question asked of an estimator.  The disparity is written but should not
    be read -- a superposition on a configuration with ten coincident points per
    mixture is dominated by a constraint nothing can satisfy.
...
#: One row per (perspective, source): a collection scored against the mixture
#: simplex, for the pooled and overlay reads analysis A does not carry.  Analysis
#: A scores the ten seeds one at a time; these score the objects those ten
#: *summarise*, so the two files together give a centre and a spread.
TRUTH_FIELDS = [
    "perspective", "taxonomy", "metric", "source", "n_points",
    "dcor", "disparity",
]

#: ``source`` values in the truth CSV, and what each one scores.  The three pool
#: rows are three different objects and are not interchangeable -- see
#: :func:`score_against` and :func:`mixture_block_means`.
TRUTH_SOURCES = (
    "overlay_mean",         # seed_mean: the mean of the ten aligned fits
    "pool_mean_embedding",  # mean_after: the joint fit's per-mixture centroids
    "pool_mean_distance",   # the 160x160 averaged down to one row per mixture
    "pool160",              # the pool itself, against a 160-point truth
)
...
def truth_for(ids, labels=None):
    """``(simplex geometry, simplex distance matrix)`` over *ids*.

    *labels* renames the rows without touching the weights, which is what lets a
    16-point truth meet a configuration labelled by **mixture** rather than by
    adapter.  Both means over seeds are labelled that way, because neither is one
    adapter; passing ``[mixture_key(m) for m in order]`` here puts the truth in
    the same vocabulary rather than leaving every downstream call to pass a
    ``key=``.

    On the 160-model pool the labels are the adapters and the weights come out
    **ten-to-one**: all ten seed-siblings share a recipe, so the truth asserts
    they are the same model and their mutual distance is exactly 0.  That is not
    an artefact to be routed around -- it is the truth's actual claim about this
    pool, and scoring against it asks how far a surrogate is from agreeing.  See
    :func:`score_against`.
    """
    weights = suite.truth_weights(ids)
    names = list(labels) if labels is not None else list(ids)
    vertex_names = [f"g{j + 1}" for j in range(weights.shape[1])]
    return (simplex_geometry(weights, names, vertex_names),
            simplex_distance_matrix(weights, names, vertex_names))


def mixture_block_means(dm):
    """The 160x160 pool averaged down to one 16x16 row per mixture.

    Cell ``(a, b)`` is the mean over the 100 adapter pairs joining mixture *a*'s
    ten seeds to mixture *b*'s.  This is the **mean distance matrix** of the
    pool, and it is a different object from the mean *embedding*: this one never
    embeds, so it is the pooled read dCor* can eat on its own full-dimensional
    terms, while ``mean_after`` is a 2-D configuration and would force dCor* onto
    a 2-D footing that analysis A's per-seed numbers are not on.

    **The diagonal is 0, not the within-mixture mean.**  A diagonal is a model's
    distance to itself and a matrix with anything else on it is not a distance
    matrix; the within-mixture spread this drops is exactly what analysis B
    measures and reports as its own cell, so nothing is lost, only moved.
    """
    ids = list(dm.model_ids)
    M = np.asarray(dm.matrix, dtype=np.float64)
    keys = np.array([mixture_key(m) for m in ids])
    order = sorted(set(keys))
    where = {k: np.flatnonzero(keys == k) for k in order}
    out = np.zeros((len(order), len(order)), dtype=np.float64)
    for a in range(len(order)):
        for b in range(a + 1, len(order)):
            block = M[np.ix_(where[order[a]], where[order[b]])]
            out[a, b] = out[b, a] = float(block.mean())
    return DistanceMatrix(matrix=out, model_ids=order, metric=dm.metric,
                          taxonomy=dm.taxonomy)


def coordinate_matrix(geo):
    """Pairwise Euclidean distances among a configuration's own points.

    The only way a *configuration* can be handed to dCor*, which reads distances
    and never embeds.  Note what this costs: the resulting number is a 2-D read,
    because the configuration is, and so it is not on the same footing as a dCor*
    computed from a surrogate's full-dimensional distances.  Used only where the
    thing being scored is a mean over embeddings and so has no distance matrix of
    its own to offer.
    """
    from scipy.spatial.distance import pdist, squareform

    coords = np.asarray(geo.coordinates, dtype=np.float64)
    return DistanceMatrix(matrix=squareform(pdist(coords)),
                          model_ids=list(geo.model_ids), metric="euclidean",
                          taxonomy=geo.taxonomy)


def score_against(tgeo, tdm, *, geo=None, dm=None):
    """``(dcor, disparity)`` for one collection against one truth.

    Exactly one of *geo* and *dm* is given.  With *geo* the object being scored
    is already a configuration -- a mean over embeddings -- so the disparity is a
    direct superposition and the dCor* goes through :func:`coordinate_matrix`.
    With *dm* it is a distance matrix, which is the footing analysis A scores on:
    the disparity embeds it first, at the truth's own dimension, and the dCor*
    reads it as it stands.

    Rows are matched by identifier in both cases, so the truth must already be
    labelled in the same vocabulary as the thing being scored -- see
    :func:`truth_for`.
    """
    if (geo is None) == (dm is None):
        raise ValueError("score_against takes exactly one of geo= and dm=")
    if geo is not None:
        return (dcor_vs_truth(coordinate_matrix(geo), tdm),
                float(procrustes_compare(tgeo, geo).disparity))
    return (dcor_vs_truth(dm, tdm),
            disparity_vs_truth(dm, tgeo, random_state=suite.MDS_SEED,
                               n_components=tgeo.coordinates.shape[1]))


def truth_row(name, spec, source, n_points, scored):
    dcor, disparity = scored
    return {
        "perspective": name, "taxonomy": spec["taxonomy"],
        "metric": spec["metric"], "source": source, "n_points": n_points,
...
def main() -> None:
    ...
    ap.add_argument("--out-truth", default="initsweep_truth.csv")
    args = ap.parse_args()
    ...
    score_rows, sep_rows, geo_rows, truth_rows = [], [], [], []
    t_start = time.time()
    for name in names:
        pspec = specs[name]
        ...
        # The overlay's centre, scored against the same truth analysis A scores
        # the ten seeds against -- so the figure's band and the point it is drawn
        # around measure the same thing on the same simplex. The truth is
        # labelled by mixture here because ``seed_mean`` is: a point of it is ten
        # adapters' aligned positions averaged, and names no one adapter.
        mix_tgeo, mix_tdm = truth_for(order, [mixture_key(m) for m in order])
        truth_rows.append(truth_row(
            name, pspec, "overlay_mean", len(seed_mean.model_ids),
            score_against(mix_tgeo, mix_tdm, geo=seed_mean)))
        ...
        # ── the pool, scored against the truth: three objects, not one ────
        # Analysis A never scores the pool, and the reason is in ``truth_for``:
        # the ten seed-siblings share a recipe, so a 160-point truth pins 720 of
        # the 12,720 pairs at exactly 0. The first two rows below step around
        # that by scoring a 16-point summary of the pool instead -- the mean
        # embedding and the mean distance matrix, which are different objects and
        # disagree because MDS is not linear. The third row does not step around
        # it: it scores the pool against that 160-point truth as it stands, which
        # asks how near the surrogate comes to the truth's claim that
        # initialisation does not exist. That is analysis B's question put to an
        # estimator instead of to a pair of means, so it belongs here and it is a
        # dCor* row -- a Procrustes fit against a configuration with ten
        # coincident points per mixture is dominated by a constraint nothing can
        # satisfy, so its disparity is written but is not drawn.
        block_dm = mixture_block_means(pool_dm)
        pool_tgeo, pool_tdm = truth_for(pool_ids)
        truth_rows += [
            truth_row(name, pspec, "pool_mean_embedding",
                      len(after_geo.model_ids),
                      score_against(mix_tgeo, mix_tdm, geo=after_geo)),
            truth_row(name, pspec, "pool_mean_distance",
                      len(block_dm.model_ids),
                      score_against(mix_tgeo, mix_tdm, dm=block_dm)),
            truth_row(name, pspec, "pool160", len(pool_ids),
                      score_against(pool_tgeo, pool_tdm, dm=pool_dm)),
        ]
        for row in truth_rows[-3:]:
            print(f"  {name:<20} {row['source']:<20} "
                  f"dcor={row['dcor']:.4f} disp={row['disparity']:.4f}",
                  flush=True)
        summary = rows[0]
    ...
    write_csv(outdir / args.out_truth, TRUTH_FIELDS, truth_rows)
