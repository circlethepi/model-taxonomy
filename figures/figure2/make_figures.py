...
yahoo: the mixture key, then one MDS panel per taxonomy level. It is the same
picture ``figures/simplex3_olmo2/fig_crosslevel_mds.png`` draws, with two
deliberate differences. The panel titles carry the **taxonomy name and nothing
else** — no dCor, no Procrustes residual, no stress — and there is no figure
title, no corpus subtitle and no title over the mixture key. Those numbers have
not gone missing; they are what the bottom row is made of, and repeating them
above it would say the same thing twice.

**Bottom row** — four readings of the Procrustes disparity against the
ground-truth mixture geometry, on one shared log y axis:

1. every **base model** on yahoo, x = taxonomy level;
2. every **corpus** on OLMo-2-1B, x = taxonomy level;
3. the **collection-size sweep**, x = models in the collection;
4. the **nsweep**, x = training draw size.
...
is better**. The axis is logarithmic because it has to span three orders of
magnitude — 0.007 for the functional level on the yahoo simplex against 0.8 for
the behavioral level at a ten-example training draw — and a linear axis would
flatten every panel but the behavioral curves.
...
::
    ...
    python figures/figure2/make_figures.py --row bottom
...
#: Marker and dash per level, so the four curves in panels 3-4 survive a
#: greyscale print and a reader who cannot separate the hues. They travel with
#: the level, not with its position.
LEVEL_MARKERS = {"Data": "o", "Structural": "s",
                 "Functional": "^", "Behavioral": "D"}
...
#: Left to right after the mixture key. Reading order of the taxonomy, which is
#: also the order panels 1-2 group their bars in.
PANEL_ORDER = ["Behavioral", "Functional", "Structural", "Data"]
...
def draw_top_row(fig, gs, levels: list[Level], cells, ids) -> None:
    ...
    for k, label in enumerate(PANEL_ORDER):
        crosslevel_panel(fig.add_subplot(gs[0, k + 1]), by_label[label].label,
                         cells[label])
...
#: Panel -> (csv, x column, disparity column). The disparity column is the
#: reading chosen for that sweep; see the module docstring for why.
SWEEPS = {
    "collection": (FIGURES_ROOT / "simplex_collection_size" / "group_size_scores.csv",
                   "n", "disparity_B", "Models in the collection"),
    "nsweep": (FIGURES_ROOT / "simplex3_nsweep_olmo2_nsweep" / "nsweep_scores.csv",
               "n_samples", "disparity_requested", "Training draw size"),
}
...
def draw_sweep(ax, which: str, levels: list[Level], legend: bool) -> None:
    ...
    path, xcol, ycol, xlabel = SWEEPS[which]
    for lv in levels:
        ...
        ax.plot(xs, med, color=lv.color, ls=LEVEL_DASHES[lv.label],
                marker=LEVEL_MARKERS[lv.label], ms=4, lw=1.4, label=lv.label,
                zorder=3)
    ax.set_xscale("log")
...
YLABEL = "Procrustes disparity vs. data mixture (lower = better)"

#: Shared y range for the whole bottom row, log scale. Fixed rather than fitted
#: so two builds of this figure — the two ``--behavioral`` variants especially —
#: can be laid side by side. It covers every value all four panels draw: the
#: functional level on the yahoo simplex at the bottom, the behavioral level at
#: a ten-example training draw at the top.
YLIM = (4e-3, 1.2)


def draw_bottom_row(fig, gs, bar_levels: list[Level],
                    sweep_levels: list[Level]) -> list:
    """The four score panels, into *gs* (1 x 4), sharing one log y axis."""
...
def build(row: str, decoding: str, cache_root=None, no_cache=False) -> Path:
    ...
    if want_top:
        ...
        r += 1
    if want_bottom:
        draw_bottom_row(fig, outer[r].subgridspec(1, 4), bar_levels, sweep_levels)
...
def main() -> None:
    ...
    args = ap.parse_args()
    print(f"wrote {build(args.row, args.behavioral, args.cache_root, args.no_cache)}")
