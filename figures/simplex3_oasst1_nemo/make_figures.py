#!/usr/bin/env python
"""The simplex3 cross-level closer for Mistral-Nemo-Instruct-2407 on oasst1 best-reply pairs.

The same four levels and the same cross-level closer as the yahoo suites, over a
different corpus. The vertex axis here is language, one per vertex -- g1 en, g2 es, g3 ru, g4 zh -- so this is a
...
The vertices are languages, so this run asks a different question from
the other two: whether a level recovers a *language* mixture, where
yahoo and dolly ask about topic and task. Note also that `zh` has 738
rows against `en`'s 7837, so an n=500 draw is 68% of the zh pool and
the seed spread at g4-heavy mixtures understates the sampling variance.
...
Usage
-----
    python figures/simplex3_oasst1_nemo/make_figures.py
    python figures/simplex3_oasst1_nemo/make_figures.py --level structural --level behavioral
"""
...
#: `03_adapters/mistralai--Mistral-Nemo-Instruct-2407` holds every adapter trained on this base model
...
def main() -> None:
    ...
    suite.run_suite(
        ...
        datasets=DATASETS,
        source="figures/simplex3_oasst1_nemo/make_figures.py",
    )
