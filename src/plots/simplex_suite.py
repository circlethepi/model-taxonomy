"""The simplex3 visualization suite: four taxonomy levels over one run's adapters.

Sixteen adapters spanning a 3-group topic simplex, on whichever base model the
caller names. Distance matrices in ``copper_r``; MDS embeddings coloured by each
model's own mixture (see :mod:`src.plots.simplex`).

This was ``scripts/make_simplex3_figures.py``, written for the qwen run and
generalized in place as the nemo, olmo2 and llama3i suites landed. It is a module
rather than a script because there are now three runs to draw and the difference
between them is a handful of arguments — :func:`run_suite` takes them, and each
run keeps a **driver** that supplies its own:

* ``scripts/make_simplex3_figures.py`` — the qwen run, at full grid width
* ``figures/simplex3_qwen_v4/make_figures.py`` — the qwen run, restricted
* ``figures/simplex3_llama3i/make_figures.py`` — the Llama-3.1-8B-Instruct run

Architecture is **derived**, never spelled here: :func:`architecture` reads the
layer count, head count, attention layout and q_proj gating off the checkpoint's
own config, so a uniform-attention model loses every ``linear-attn`` surrogate
and the q_proj query/gate split by producing a *smaller* figure set rather than
by taking a special case. ``scripts/check_analysis.py`` pins that degradation on
both the Qwen3.5-4B and Llama-3.1-8B-Instruct layouts.
...
Restricting a run
-----------------
``run_suite(select=...)`` narrows a level to a chosen set of **perspectives** —
``(surrogate, metric)`` pairs. :func:`compact_selection` turns that set into the
smallest rectangle covering it, and the restriction is applied to the row mapping
and the metric columns *before* anything is computed, so a restricted run is
cheaper and not merely narrower. A restricted level emits one compact grid;
unrestricted, functional emits two figures and structural three, split by how
their surrogates are built.
...
#: The checkout this module lives in.  ``parents[2]`` because the file sits at
#: ``src/plots/simplex_suite.py``; it was ``parents[1]`` while this was a script
#: under ``scripts/``, and :func:`_default_cache_root` derives paths from it.
...
#: What a stored collection's ``config.json`` records as having written it.
#: Set by :func:`run_suite` from the *driver* that called it, not fixed to this
#: module: three drivers now share this code, and "which run wrote this" is the
#: question the provenance field exists to answer.
SOURCE = "src/plots/simplex_suite.py"
...
def metric_row(idx, taxonomy, ids, tf, blocked=None, label=None, cols=None,
               **selectors):
    """One grid row: every metric column at a single fixed selector.

    *cols* restricts which metric columns are computed, defaulting to all of
    ``METRIC_COLS``.  It is what makes a restricted suite cheaper rather than
    merely narrower: the columns are dropped before the distances are computed,
    not after.  The representations are still resolved once and shared, so a
    one-column row costs the same read as a seven-column one.
    ...
    *blocked* maps a column to the reason it has no value here, so a structural
    ...
    """
    blocked = blocked or {}
    cols = METRIC_COLS if cols is None else list(cols)
...
def behavioral_rows(surrogates=True):
    """``(selector, transform)`` per row, split out so a driver can restrict it.

    The other three levels already exposed their row mapping — ``functional_group_rows``,
    ``dataset_rows``, the structural spec builders — and behavioral was the one
    that built its rows inside its cell builder, so it was the one level a caller
    could not narrow before anything was computed.
    """
...
def behavioral_cells(idx, ids, surrogates=True, rows=None, cols=None):
    rows = behavioral_rows(surrogates) if rows is None else rows
    cells = {}
    for row, (selector, tf) in rows.items():
        ...
        got = metric_row(idx, "behavioral", ids, tf, blocked, label=row,
                         cols=cols, behavioral_selector=selector)
...
def functional_cells(idx, ids, rows, cols=None):
    cells = {}
    for row, (selector, tf) in rows.items():
        got = metric_row(idx, "functional", ids, tf, _blocked(False, tf),
                         label=row, cols=cols, functional_selector=selector)
        cells.update({(row, c): v for c, v in got.items()})
    return list(rows), cells
...
def _load_weights(names, layers, projs):
    """The fleet's LoRA factors, read from **this base model's** adapter directory.

    Scoped to ``ADAPTER_ROOT / BASE_SLUG`` rather than to ``03_adapters``, and that
    is a correctness fix rather than a tidy-up. Every simplex3 suite trains the
    same sixteen recipes, so the adapter *directory names* are identical across
    base models — ``yahoo_000g1_000g2_100g3_…_b5008_fea27ccee`` exists under
    ``Qwen--Qwen3.5-4B`` and under ``meta-llama--Llama-3.1-8B-Instruct`` alike.
    Handed the unscoped root, :func:`_find_safetensors` falls back to scanning one
    level down and returns the *first* slug that has the name, and ``sorted()``
    puts ``Qwen--…`` ahead of ``meta-llama--…`` because ``Q`` < ``m``. So the
    llama structural level silently read qwen's weights and scored 0.9683 —
    qwen's own number to four decimals, which is what gave it away.

    Scoping makes the flat ``root/name/adapter_model.safetensors`` branch hit
    directly, so the ambiguous scan is never reached.
    """
    from src.notebook.lora_weights import load_lora_weights
    return load_lora_weights(
        names, ADAPTER_ROOT / BASE_SLUG, layer_indices=list(layers),
        projections=list(projs), attn_num_heads=ATTN_NUM_HEADS)


def _structural_grid(idx, names, ids, specs, cols=None):
    ...
    *before* anything is stored, so a stored structural matrix is addressed in
    ...
    for row, (layers, projs) in specs.items():
        print(f"    {row}")
        for col in (METRIC_COLS if cols is None else cols):
            if col in DISTRIBUTIONAL:
...
def compact_selection(rows, allowed):
    """The smallest rectangle covering *allowed*, as ``(rows, cols)``.

    *rows* is a level's full row mapping — ``{label: spec}`` from
    :func:`behavioral_rows`, :func:`functional_group_rows`, :func:`dataset_rows`
    or one of the structural spec builders. *allowed* is an iterable of
    ``(surrogate, metric)`` **perspectives** to keep.

    Returns the subset of *rows* carrying at least one selected perspective, in
    the builder's own order, together with the metric columns likewise, in
    ``METRIC_COLS`` order. Both orders come from the builders rather than from
    *allowed*, so a restricted figure lists its rows and columns exactly where an
    unrestricted one would and the two stay readable side by side.

    A surrogate named in *allowed* that the level does not build raises. That is
    the point rather than an inconvenience: these selections are carried across
    architectures, and a hybrid-only surrogate asked of a uniform-attention model
    should say so here rather than silently produce a smaller figure than the one
    that was asked for. Callers doing the carrying are expected to have mapped
    the labels already; see ``figures/simplex3_llama3i/make_figures.py``.
    """
    allowed = list(allowed)
    missing = sorted({r for r, _ in allowed} - set(rows))
    if missing:
        raise KeyError(
            f"no such surrogate in this level: {missing}. This level builds "
            f"{sorted(rows)}. A surrogate carried over from another run may not "
            "exist for this architecture — map it or drop it in the driver."
        )
    unknown = sorted({c for _, c in allowed} - set(METRIC_COLS))
    if unknown:
        raise KeyError(f"no such metric: {unknown}. Known: {METRIC_COLS}")

    keep_rows = {r for r, _ in allowed}
    keep_cols = {c for _, c in allowed}
    return ({k: v for k, v in rows.items() if k in keep_rows},
            [c for c in METRIC_COLS if c in keep_cols])


# ── Figures ───────────────────────────────────────────────────────────────────

def emit(level, rows, cells, outdir, title, cols=None):
    cols = METRIC_COLS if cols is None else list(cols)
    dm_grid(cells, rows, cols, f"{title} — distance matrices",
            savepath=outdir / f"fig_{level}_dm_grid.png")
    plt.close("all")
    mds_grid(cells, rows, cols, f"{title} — MDS embeddings",
             savepath=outdir / f"fig_{level}_mds_grid.png")
    plt.close("all")


def emit_detail(level, row, cells, outdir, title, cols=None):
...
#: Per level: the figure title and the label the per-metric detail panels carry.
#: Kept in one place so a driver names a level rather than adding a branch.
LEVEL_TITLES = {
    "behavioral": ("Behavioral level", "Behavioral"),
    "functional": ("Functional level", "Functional"),
    "structural": ("Structural level", "Structural"),
    "dataset_embedding": ("Dataset-embedding level", "Dataset embedding"),
}

ALL_LEVELS = ["behavioral", "functional", "structural", "dataset_embedding"]


def _selected_level(idx, ids, names, level, chosen, surrogates):
    """Build one level restricted to *chosen* perspectives -> ``(rows, cols, cells)``.

    Every level collapses to a **single** compact grid here. Unrestricted,
    functional emits two figures and structural three, split by how their
    surrogates are constructed; a selection cuts across those groupings, and the
    seam has no meaning for a reader of the restricted figure. So the full row
    mapping is assembled across the builders first and narrowed by
    :func:`compact_selection`, which also means a surrogate is looked up in one
    namespace whichever builder produced it.
    """
    if level == "behavioral":
        full = behavioral_rows(surrogates)
        rows, cols = compact_selection(full, chosen)
        return rows, cols, behavioral_cells(idx, ids, rows=rows, cols=cols)[1]

    if level == "functional":
        full = {k: (v, None) for k, v in functional_layer_rows().items()}
        full.update(functional_group_rows(surrogates))
        rows, cols = compact_selection(full, chosen)
        return rows, cols, functional_cells(idx, ids, rows, cols=cols)[1]

    if level == "dataset_embedding":
        full = dataset_rows(idx, ids, surrogates)
        rows, cols = compact_selection(full, chosen)
        return rows, cols, dataset_cells(idx, ids, rows, cols=cols)[1]

    if level == "structural":
        full = {**structural_layer_specs(), **structural_group_specs(),
                **structural_projection_specs()}
        rows, cols = compact_selection(full, chosen)
        return rows, cols, _structural_grid(idx, names, ids, rows, cols=cols)[1]
...
def run_suite(*, base_model, draw, outdir, cache_root=None, levels=None,
    ...
    qwen driver, the restricted qwen driver and the llama3i driver are three
    calls rather than three copies of 90 lines.

    *select* maps a level to the ``(surrogate, metric)`` perspectives it is
    restricted to; a level absent from it is built in full, which is what keeps
    ...
    *crosslevel_only* keeps the cross-level closer and drops every other output:
    ...
    SUITE_CACHE = SuiteCache(CACHE_ROOT, read=not no_cache)
    ...
    for level in levels:
        title, detail_title = LEVEL_TITLES[level]

        if level in select:
            chosen = list(select[level])
